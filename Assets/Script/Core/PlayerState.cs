...
public class PlayerState : NetworkBehaviour
{
    ...
    void ApplyMode(PlayerMode mode)
    {
        ...
        if (IsOwner)
        {
            ...
            {
                CameraManager.Instance.SetMode(CameraMode.Owner);
                StartCoroutine(RegisterCamera());
            }
            else if (mode == PlayerMode.Board)
            {
                CameraManager.Instance.SetMode(CameraMode.Turn);

                StartCoroutine(ForceBoardCamera());
            }
        }
    ...
    IEnumerator ForceBoardCamera()
    {
        ...
        {
            GameManager.Instance.ForceUpdateTurnCamera();
        }
    }
    IEnumerator RegisterCamera()
    {
        ...
        if (CameraManager.Instance.GetMode() != CameraMode.Owner)
            yield break;
